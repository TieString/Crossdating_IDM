...
#### 28.1 `reviewOnly` 的手动确认边界

`planDiagnosisEventEdit()` 的默认严格路径继续拒绝 `reviewOnly`，因此后台、自举和基准脚本不能自动执行降低门槛的窗口。建议卡中的“应用”是用户结合图表和实体样芯作出的明确确认，单独调用 `planManuallyConfirmedDiagnosisEventEdit()`；它只解除 `reviewOnly` 门，stale、窗口范围、方向和编辑范围等其余安全检查保持不变。
...
13. `reviewOnly` 不允许被后台或自举流程自动应用；用户在建议卡中明确确认后可以应用所选年份。
14. 手动确认只解除 `reviewOnly` 门，不绕过 stale、窗口范围、位移方向或编辑范围检查。
