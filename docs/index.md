...
树轮交叉定年需要在序列图形判读、年份校正和统计质量控制之间反复迭代，而诊断结果与数据编辑相互分离会增加版本管理和问题定位成本。本文开发了 Crossdating-IDM，一款面向 RWL 数据的交互式桌面程序。系统在同一工作区内整合多格式数据导入、Tucson 格式透明编辑、宽度网格与折线图联动、扫描影像辅助判读、COFECHA 运行与报告解析、动态参考序列、事件级定年建议以及操作审计。COFECHA 报告中的序列和问题年份可直接定位到工作数据，所有校正均经统一编辑器写入，并触发新的诊断状态。

定年建议模块以整体滞后、重叠分段相关、受约束整数滞后路径和可执行编辑反事实为核心证据，将异常表示为缺轮、伪轮、局部区段移动和整条序列移动四类事件。系统联合原始宽度、一阶差分、预白化和 COFECHA 标准化视图，汇总主参考序列与逐参考芯证据，并输出事件类型、位移量和较窄的复核窗口。候选经用户确认后一次应用一个事件，随后基于更新后的工作序列重新诊断，从而形成“识别—定位—编辑—复核”的连续闭环。当前运行时由 TypeScript 在 Web Worker 中完成全部诊断与静态树模型推断。

在冻结的 ITRDB 独立测试集中，25 个文件的 500 条目标序列构成 2,500 个案例，共包含 4,797 个真值事件。4,187 次实际复核前沿诊断中，工作流建议准确率为 88.73%，严格操作类型准确率为 89.09%，工作流等价操作准确率为 93.50%。单事件缺轮、伪轮、局部移动和整体移动的建议准确率分别为 94.40%、87.20%、88.00% 和 97.60%；局部事件在系统给出正确操作时的窗口覆盖率为 94.32%。500 个无事件对照中误报 2 个，特异度为 99.60%，保存后重新打开的数据状态一致率为 100%。结果表明，Crossdating-IDM 可将分散的统计异常转化为位置明确、可执行且可追踪的交叉定年修订建议。

**关键词**：树轮交叉定年；事件诊断；RWL；COFECHA；可审计编辑

## 引言

### 树轮交叉定年与质量控制

树轮交叉定年通过比较不同样芯中宽轮和窄轮的共同变化，将每一轮精确赋予日历年份，是树轮年代学推断的基础。年轮宽度不仅包含年龄趋势，还记录气候、竞争和扰动等共同或个体生长信号；因此，可靠定年依赖多样芯之间的同步模式，而不是单条序列中的局部相似。经典的骨架图、相关分析和主年表比较共同构成了视觉与统计交叉定年的证据体系（Fritts, 1976; Holmes, 1983; Grissino-Mayer, 2001）。

实际样品中的年份不一致主要来自四类结构性事件：未形成或未被测得的缺轮、年内密度变化或误分造成的伪轮、断裂或腐朽造成的局部连续缺失，以及树皮侧缺失或起始年份错误造成的整体年代偏移。缺轮和伪轮会使事件前后的相对滞后发生单位跃迁，连续缺失会产生幅度更大的局部阶跃，整体偏移则在全序列维持近似恒定的滞后。简单滑动相关能够识别恒定位移，却难以同时给出结构变化的类型和位置；编辑距离与动态规划研究因此将缺轮和双轮作为序列编辑事件处理（Van Deusen, 1990; Wenk, 2003）。在样品解释层面，局部移动常与腐朽、芯段断裂或拼接缺口对应，整体负向移动常提示树木死亡早于参考序列末端、树皮或边材缺失以及树皮侧断裂导致的外层年份丢失（Bannister, 1962; Pearson, 1997）。

参考年表把多个可靠样芯的共同变化聚合为连续时间基线，并通过样本深度降低个体生长噪声。目标序列与参考年表的整体相关可定位恒定偏移，重叠区段相关可显示局部失配，而逐参考芯一致性可判断异常是否由单一参考序列主导。因而，有效的质量控制不仅要指出“相关性低”，还应把多尺度一致性转化为事件类型、位移方向、候选位置和编辑后的改善证据。

### 现有软件与分析工作流

树轮软件已分别覆盖测量、质量控制、统计分析和年表管理等环节。COFECHA 通过标准化、分段相关和交叉相关检查序列定年与测量质量，并长期作为树轮数据质量控制的核心工具（Holmes, 1983; Grissino-Mayer, 2001）。dplR 将 RWL 读取、去趋势、年表构建、移动相关、交叉相关和骨架图引入 R 环境，使 COFECHA 风格的诊断可以在可复现脚本中扩展（Bunn, 2008, 2010）；ARSTAN 则集中处理去趋势、预白化和年表标准化（Cook and Krusic, 2005）。CooRecorder/CDendro 连接扫描影像、年轮宽度测量和人工交叉定年，为影像驱动的测量流程提供了成熟工具（Maxwell and Larsson, 2021）。

现有研究也从算法层面扩展了定年匹配。Baillie 和 Pilcher（1973）的滑动匹配奠定了基于候选偏移搜索的计算框架；Van Deusen（1990）和 Wenk（2003）进一步把缺轮和伪轮纳入动态规划或编辑距离；Hassan 等（2019）用贝叶斯模型表达不同偏移的相对支持度。近期的 COFECHA 自动化工作提高了未定年序列批量匹配和重复运行的效率（Schwartz, 2025, 2026）。这些方法分别强化了偏移搜索、局部编辑或候选排序，但其统计输出通常仍需人工转录为具体的树轮数据编辑操作。

集成式软件开始处理工具切换和数据版本问题。RWLApp 将格式转换、COFECHA 辅助检查、交互修改、合并年表维护与审计记录连接为连续流程，显示了端到端数据管理对树轮实验室工作的直接价值（Bakhtiyorov et al., 2026）。在此基础上，交叉定年工作流仍需要进一步连接“问题区段”与“可执行事件”：诊断结果应直接说明应插入缺失年份、删除伪轮、移动哪一段或移动整条序列，并在同一数据状态上验证编辑前后的证据变化。

### 工作流需求

面向实际复核的交叉定年程序需要同时保持数据、参考和诊断状态的一致性。首先，程序应识别实验室常见的 RWL 及交换格式，并在编辑 Tucson 文件时保留编号字段宽度和终止标记等原始格式信息。其次，宽度表、序列图、扫描影像和 COFECHA 报告应共享同一序列—年份坐标，使统计异常能够直接落到待复核单元。第三，参考序列应能由用户选择的可靠样芯生成，也能随 COFECHA 结果更新，并保留样本深度和来源信息。

传统的折线图人工比对本质上是一种开放式搜索：专家需要反复平移序列、尝试插入或删除年份、重新比较波形，再决定下一次尝试；重复运行质量控制程序和跨工具检查已被指出是树轮工作流中的主要操作瓶颈（Schwartz, 2025; Bakhtiyorov et al., 2026）。单一且特征明显的缺轮或伪轮通常可以较快确认；当序列包含混合事件、大距离整体错位或断点未知的局部移动时，候选位置和操作会迅速增多，专家难以预先判断还需多少轮比较才能找到合适的编辑位置。与此同时，多个候选位置、操作方向和中间数据状态需要被同时记忆和比较，形成较高的认知负担。事件级诊断将这种次数未知的试错过程压缩为操作类型、位移量和聚焦窗口明确的复核任务，减少无结构的重复比较，使专家能够把注意力集中在样品证据和最终判断上。

更关键的需求是把异常识别连续推进到编辑决策。一个建议需要同时回答四个问题：是否存在事件、属于哪类操作、位移量是多少、应在何处复核。对多事件序列，系统还需在每次编辑后重新计算当前问题前沿，避免旧候选继续作用于已改变的数据。最后，原始数据、工作数据、撤销/恢复状态和每次建议的证据应共同进入操作记录，使保存、重开和重复 COFECHA 分析均可追溯到明确的数据版本。

### 研究目标与主要贡献

本文的目标是构建一个面向树轮交叉定年的集成式桌面工作区，并将统计诊断推进为事件级辅助决策。Crossdating-IDM 的主要贡献包括三方面。第一，系统在统一工作状态中连接多格式导入、Tucson 格式透明编辑、联动可视化、扫描影像、COFECHA 和操作审计，减少跨工具传递中的版本分离。第二，系统建立了由全局滑动匹配、重叠分段诊断、受约束滞后路径、可执行反事实和多参考芯证据组成的事件诊断流程，直接输出缺轮、伪轮、局部移动或整体移动及其复核窗口。第三，系统以一次确认一个事件、编辑后立即重新诊断的方式处理复杂事件链，并在冻结的 ITRDB 文件级独立测试中量化操作准确率、窗口覆盖率、连续恢复率、无事件特异度和保存重开稳定性。

Crossdating-IDM 的定位是衔接 COFECHA 与专家判读的事件级辅助决策工具，而不是替代 COFECHA 或自动完成交叉定年。COFECHA 继续提供标准化主序列、分段相关和质量控制结果；本程序将这些结果与内部事件证据组织为位置和操作明确的复核建议。建议只有在专家结合折线、扫描影像和样品状态确认后才进入工作数据，随后再运行 COFECHA 检查修订后的序列。
...
事件建议引擎在运行时完全由 TypeScript 执行。规则证据、Viterbi 滞后路径、反事实扫描和静态树模型排序均在 Web Worker 内完成；离线固化的树模型参数以静态资源载入并直接推断。该结构将交互界面、数据状态、诊断计算和 COFECHA 调用分离，同时通过工作区版本号和数据哈希保证各层使用同一份工作序列。

### 工作流程

系统工作流由九个相连步骤组成：（1）打开并自动解析测量文件；（2）建立原始基线和可编辑工作副本；（3）在宽度网格、折线图和扫描影像中检查序列；（4）运行 COFECHA 或手动选择可靠样芯以获得参考序列；（5）选择目标序列并启动事件诊断；（6）查看事件类型、位移量、复核窗口和证据变化；（7）结合联动图形与报告确认候选；（8）通过统一编辑器应用一个事件并记录操作；（9）在更新后的工作序列上重新诊断，并运行 COFECHA 完成下一轮质量控制。该循环持续到当前序列不再出现待处理事件。
...
**交互式宽度编辑。** 宽度网格支持数值修改、缺失年份插入、伪轮删除与相邻宽度合并、年份区间移动和整条序列移动。所有操作复用同一编辑器接口，并进入统一历史快照；用户可以逐步撤销、恢复或直接回到首次载入的原始基线。诊断建议应用时写入事件类型、候选年份、移动方向、选定区间、编辑前后指标和 `auto-suggested` 来源，使自动建议与人工编辑共享同一审计结构。
...
**COFECHA 集成与参考序列。** 系统直接针对当前工作数据运行 COFECHA，并解析 PART 3 主定年序列、PART 6 问题标记和 PART 7 统计结果。正常路径优先采用 PART 3 master dating series 作为动态参考；PART 6 中无 A 标记的样芯归入 `anchor_pass`，带 A 标记的样芯归入 `candidate_flagged`，用于确定后续重点检查对象。当可用锚定芯少于 3 条时，系统通过样芯间零滞后相关聚类构建临时参考。临时参考对每条锚定芯执行 32 年、50% 响应的三次平滑样条去趋势、指数化、AR(p) 预白化和逐年平均，再标准化为均值 0、标准差 1 的残差年表。用户也可手动多选可靠样芯，按日历年对齐求算术平均；工作数据改变后，动态参考被标记为待更新，并在下一次 COFECHA 运行后刷新。

**事件级诊断。** 诊断模块读取目标序列、参考年表、逐参考芯集合和当前 COFECHA 分类，生成全局滑动、传播模式和分段异常候选。候选依次经过受约束滞后路径、局部编辑对齐、全区间反事实定位、多视图与逐参考芯融合、静态树模型窗口排序和联合事件裁决，最终形成一个当前复核前沿建议。建议包含操作类型、位移量、复核窗口、相对证据分数以及编辑前后相关和问题段变化。
...
令 \(g_t\) 表示目标序列在年份 \(t\) 附近相对参考序列的最佳整数滞后，负值表示目标数据需要向较早的日历年份移动。系统将诊断对象定义为四类可直接映射到编辑器的事件，并把无事件状态作为对照。缺轮和伪轮对应单位滞后转移，局部移动对应两个稳定平台之间幅度不小于 2 年的转移，整体移动对应全序列近似恒定的非零滞后。已有的零值被保留为明确的缺轮标记，在相关和参考序列计算中按 absent ring 处理。
...
| 局部移动（Partial move） | 序列内部存在两个长稳定滞后平台，断点处出现幅度 \(|q|\ge 2\) 的阶跃 | 芯段腐朽、断裂、局部连续年轮缺失或分段拼接偏移 | 给出位移量、断点和受影响区间，执行 `partialRangeMove` |
| 整体移动（Whole-series move） | 全序列在同一非零滞后达到稳定最优匹配，内部无主要断点 | 树木死亡早于参考序列末端；树皮、边材或外层木质部缺失；树皮侧断裂导致末端年份丢失 | 给出整序列位移量，执行 `wholeSeriesMove` |

上述定义把统计模式与物理编辑统一在同一事件语义中。事件诊断输出的年份是需要复核和执行操作的日历位置，而不是单纯的低相关区段中心；局部移动还同时保存被移动区间和显式缺失区间，以保持连续缺段的结构信息。

### 事件诊断证据

**参考与标准化证据。** 目标序列首先与当前动态或手动参考对齐。动态路径优先使用 COFECHA PART 3 主定年序列，逐参考芯证据则来自参考配置中的源样芯；计算单芯投票时排除目标芯。临时参考和多视图诊断使用 COFECHA 风格标准化：原始宽度经 32 年、50% 响应的三次平滑样条去趋势，得到无量纲指数；随后用 Yule–Walker 方程估计 AR(p)，在 1—5 阶中以 AIC 选择阶数并生成预白化残差。默认使用对数变换而不执行一阶差分，零值不进入参考年表。该过程保留原始宽度、差分、预白化和最终残差年表四种互补视图。

**全局滑动匹配。** 全局层沿用 Baillie–Pilcher 式候选偏移搜索，先在 -100 至 +100年范围内计算目标与参考的 Pearson 相关剖面 $r(g)$，用于比较零滞后两侧的匹配结构和确定全局基线；正向 lag 不进入自动移动事件，最终移动建议仅保留最多 100 年的负向修正。每个偏移至少需要 25 个重叠年份，并记录当前滞后、最佳滞后、重叠数、相关增益和$t(g)=r(g)\sqrt{\frac{n(g)-2}{1-r(g)^2}}$,其中 $n(g)$为有效配对数。稳定且显著优于零滞后的全局峰值形成整体移动的主要证据，也为局部诊断提供树皮侧固定基线和候选位移范围。

**重叠分段诊断。** 系统以 50 年窗口、25 年步长生成主问题段，并以 30 年窗口、15 年步长补充细尺度证据。每段比较当前相关和局部最佳非零滞后相关，统一记录有效样本数、Fisher-z 变化和 t-like 改善。当前相关低且滞后搜索不能形成有意义改善的区段记为 A-like；非零滞后带来足够相关与 t-like 增益的区段记为 B-like。问题段按较老侧、较新侧、全序列或跨边界传播模式汇总，从而区分局部异常、恒定整体偏移和由单个事件向后传播的年份错位。

**受约束滞后路径。** 为把离散问题段连接为事件结构，系统在整数滞后状态上求解分段 Viterbi 路径。目标函数联合局部相关收益、状态持续长度和转移惩罚，默认要求约 10 年的稳定运行段，因而形成“长平台—稀疏转移”的滞后拓扑。路径直接提供转移前后滞后 \(g_{before}\) 与 \(g_{after}\)、变化幅度和变点年份。恒定平台支持整体移动；单个单位阶跃支持缺轮或伪轮；幅度较大的单阶跃支持局部移动；多个同向单位阶跃构成多缺轮或多伪轮阶梯。整数平台和稀疏变点始终锚定在明确的日历坐标上，可直接保留物理操作方向。

**可执行反事实。** 全局峰值、分段传播模式和滞后路径分别生成候选草案。系统针对每一草案在内存中模拟插入缺失年份、删除伪轮、移动局部区间或移动整条序列，并在虚拟编辑后的完整序列上重新运行诊断。候选证据包括全局相关、平均分段相关、目标窗口局部相关、A-like/B-like 数量、滞后回零程度、传播范围和边界清晰度的前后变化。局部编辑对齐使用带状路径限制插入/删除数量和偏离范围，用于补充单位事件候选。只有能同时改善匹配且形成合法编辑的草案进入后续定位和排序。

**事件定位与多视图联合。** 缺轮和伪轮先在约 25 年高召回区间内执行逐年插入或删除扫描，得到每个年份的反事实增益曲线。定位器同时计算原始宽度、一阶差分、AR(1) 预白化和 COFECHA 标准化视图的局部极值、累计变化点和转移证据。局部移动则联合搜索位移量与断点，并比较断点两侧的 fixed-lag 平台。多视图候选按位置邻近性聚类，再由逐参考芯投票比例、证据集中度和远距离次峰间隔确定主位置模式。

**窗口与相对证据排序。** 粗定位结果进入由 TypeScript 直接推断的静态树模型排序器；模型结合反事实曲线形状、跨视图一致性、参考芯投票、边界上下文和候选间隔，选择 5、7、9 或 13 年复核窗口。物理剖面规则进一步校正窗口中心，使其覆盖真实插入点、删除点或移动断点。候选总分经 softmax 转换为同一轮候选之间的相对证据分布，并输出名次、相对分值和证据等级。该结果使使用者能够先检查最集中的年份区间，再查看完整的编辑前后证据。

以窄窗口而非强制唯一年份作为主要输出，是事件级辅助决策的设计选择。真实缺轮、伪轮和芯段缺失需要结合样品表面、树皮端状态、扫描影像和测量轨迹判断；仅凭宽度序列，相邻数年内的编辑有时会产生接近的相关增益和相同的最终年份结构，统计证据不足以稳定地区分唯一操作年。系统因此保留窗口内的年份排序，把开放式全序列搜索收缩为 5、7、9 或 13 年的聚焦复核范围，同时由专家确定真实事件位置。任何候选均不自动写入 RWL 数据，确认后的单次编辑才会触发下一轮诊断和 COFECHA 检查。

### 复杂事件区分

COFECHA PART 6[A] 对被标记区段逐年列出当前位置前后 −10 至 +10 年的相关系数；A flag 表示当前相关偏低且在该范围内未找到更高相关位置，B flag 表示在该范围内存在更高相关位置（Grissino-Mayer, 2001）。因此，PART 6[A] 能够有效提示单位或小幅偏移，但对超过 10 年的事件只能显示当前区段异常，不能直接给出范围外的最佳位移。Crossdating-IDM 将自动移动建议在符合物理缺失语义的负向扩展至 100 年：局部移动枚举 −2 至 −100 年，整体移动仅接受全局相关剖面支持的负向位移。内部保留的正向全局相关值只用于比较方向和识别基线，不生成正向局部或整体移动建议。由此，超过 10 年的右向错位仍可获得明确的向左修正建议。
...
图 2　复杂事件的滞后拓扑与识别逻辑。恒定滞后、单阶跃、单位阶梯、相邻反向阶跃及“全局基线＋局部阶跃”分别对应整体移动、局部移动、离散同向事件、相邻抵消事件和整体—局部组合；多视图反事实与逐参考芯投票用于确定操作类型和位置。
...
本程序的价值在于把次数未知的人工试错收缩为操作类型和窄窗口明确的专家复核，而不是输出自动交叉定年结论。COFECHA 负责标准化质量控制和修订后复查，Crossdating-IDM 负责组织事件证据、缩小搜索范围并记录编辑过程，真实事件的确认仍由专家结合样品完成。二者共同构成从统计异常、样品判读到可追踪修订的连续工作流。
