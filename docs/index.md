...
系统工作流由九个相连步骤组成：（1）打开并自动解析测量文件；（2）建立原始基线和可编辑工作副本；（3）在宽度网格、折线图和扫描影像中检查序列；（4）运行 COFECHA 以获得参考序列；（5）选择目标序列并启动事件诊断；（6）查看事件类型、位移量、复核窗口和证据变化；（7）结合联动图形与报告确认候选；（8）通过统一编辑器应用一个事件并记录操作；（9）在更新后的工作序列上重新诊断，并运行 COFECHA 完成下一轮质量控制。该循环持续到当前序列不再出现待处理事件。
