...
状态：Round 6E，已修复连续局部缺块被离散缺轮阶梯覆盖；下一轮处理 B8/B9 单位事件与 partial 共存
...
### Round 6D：用双事件完成态判别双伪轮方向

仅把 lag path 扩展为有符号 `+2 -> +1 -> 0` 仍不足以决定操作方向。以 `mon151` 的
1685/1694 双伪轮为例，目标排除后的动态参考同时形成一个更高增益的负向别名路径；而只虚拟
删除或插入一次时，第二个尚未纠正的事件继续污染相关，单步 missing 反事实甚至优于正确的
false 反事实。因此不能用“路径总增益最高”或“一次编辑收益最高”直接覆盖操作类型。

本轮把原缺轮阶梯 DP 抽成有符号单位阶梯定位器，分别保留 `-N...-1 -> 0` 与
`+N...+1 -> 0`，但真正的方向门禁改为比较两个事件都纠正后的完成态：

1. 在正、负阶梯头附近分别枚举两次删除与两次插入的日历年对；
2. 用共享 master 的 raw、差分和预白化相关保留每个方向最强的 12 个完成态；
3. 每条可靠参考芯独立比较自己最支持的“双删除”与“双插入”，再稳健合并支持比例、中位数和
   下四分位数；
4. 只有 master 与至少 8 条参考芯同时明确支持删除方向，且 COFECHA 给出累计 `+2` 候选时，
   才恢复当前最前沿的一个 falseRing；UI 仍只得到一个 7 年主窗口，不暴露候选年对；
5. 窗口直接围绕有符号阶梯头构造，不再交给会在两个远距离别名之间跳模态的通用定位器。

完整结果位于
`D:\软件测试\co612-operation-composition-results\b7-false-pair-full-paired-direction-dev-2026-08-09`。
同一 415 项 manifest、10 条样芯、120 个双伪轮场景下，源 SHA 前后不变、错误 0、保存重开
100%、clean review 保持 2/55，240 个单伪轮控制的操作 199、窗口 188、Top1 59 均完全不变。

| B7 指标 | Round 6B 冻结基线 | Round 6D |
| --- | ---: | ---: |
| 响应 | 114/120 | 115/120 |
| 操作正确 | 14/120 | 65/120 |
| 主窗口覆盖 | 11/120 | 60/120 |
| Top1 | 5/120 | 21/120 |
| missingRing 反向误判 | 73/120 | 45/120 |
| wholeSeriesMove 误判 | 18/120 | 0/120 |
| partialMove 误判 | 9/120 | 5/120 |
| 完整串行恢复 | 6/120 | 45/120 |

按间距看，gap=3/9/21 的操作正确分别由 1/2/11 提升到 13/22/25，窗口由 1/1/9 提升到
13/21/23；gap=1 只有 5/30 可判为 falseRing，继续标记为操作不可辨识，不通过放宽方向门槛
制造答案。全批耗时 366.5 秒，冻结基线为约 327 秒；额外完成态枚举只在 COFECHA 明确给出
累计 `+2` 且普通 falseRing 尚未恢复时触发，普通诊断不承担该成本。

保护验证包括 co612 多事件/物理 partial `-2...-100` 19/19、ZSL 操作类型与保存重诊断
31/31、跨站点混合事件 3/3；其中 84 个“whole + 局部/单位事件”仍全部恢复正确 whole 位移，
生产构建通过。剩余主要失败是 gap=1 的不可辨识重叠，以及旧侧完成态仍被负向别名吸引；它们
不得用当前阈值继续外推，后续与单位事件/partial 组合分开处理。

### Round 6E：完整局部移动与离散缺轮阶梯的完成态竞争

ZSL `crossdated.rwl` 的 truth-blind `partialMove -4` 矩阵暴露了另一个相反方向的压缩错误：
算法已经形成 hard-gated partial 候选，但最终 sequential missing 恢复器把同一连续缺块解释为
多次单位插年，再只向用户显示一个 `missingRing`。典型案例为 ZSL091、ZSL092 和 ZSL171 的旧侧
断点；修复前分别显示 1828--1840、1828--1840 和 1841--1853 的 missingRing 窗口。

根因不是 partial 候选缺失，而是比较层级不对称：旧代码用“一次插年”与“尚未完成的 partial”
竞争，其他 lag 阶梯仍留在序列中；而一旦错误单位事件先进入 `detected`，`hasExistingUnitEvent`
又会禁止 partial 推翻它，形成循环否决。本轮改为比较两个完整解释：

1. 从 lag path 保留每个非最终状态的显示帧转移年份；离散解释一次虚拟插入全部单位事件，连续
   解释则对 hard-gated partial 候选的每个位移量与断点联合纠正。
2. 断点只在 lag 转移覆盖区和候选锚点附近逐年搜索；最近 `|shift|` 个转移年份的中位数作为
   局部边界先验，精排限制在其左右 2 年，避免长旧侧相关把断点吸到搜索区边缘。
3. 每条参考芯分别比较完整 partial 与完整 staircase；绝对 margin 不超过 `1e-9` 的平票参考
   不计入分母。自动恢复要求至少 8 条有效参考、partial 支持率至少 80%、支持票至少多 5、
   中位数小于 0 且上四分位不大于 0。
4. “是否为连续 partial”和“具体位移量”分开取证：完整家族竞争决定操作族；若只有一个
   COFECHA 分段 lag 通过 hard gate，则由它保留精确幅度，不能把明确的 `-4` 静默近似成相关
   别名 `-5`。没有唯一 COFECHA 幅度时才使用完成态位移剖面。
5. 新路径不允许覆盖 whole，也不覆盖与已确认历史分离的 missing 模式；UI 仍只得到一个
   firstFixedYear、一个 shiftYears 和一个 13 年主窗口，不暴露内部位移或断点列表。
6. 参考芯评分按“纠正结果 × 参考芯”缓存，位移选择与断点选择复用同一证据表。相同 84 状态
   子矩阵的本机墙钟时间由约 52.5 秒降至 42.1 秒；该数值含 COFECHA，仅用于同机相对比较。

完整 ZSL 结果位于
`D:\软件测试\ZSL\window-coverage-results\zsl-whole-partial4-completed-family-fix-full-2026-08-09`，
输入 SHA-256 为
`a836e8a09030ebdc09135f24f44215ca1ab0c6590fdae6827661ed7d9c64f358`。239 个唯一状态错误 0、
残差真值不匹配 0、源文件前后不变。与同提交的修复前审计相比：

| ZSL `partial -4` 指标 | 修复前 | Round 6E |
| --- | ---: | ---: |
| 纯局部操作正确 | 15/30 | **19/30** |
| 纯局部主窗口覆盖 | 13/30 | **17/30** |
| 旧侧操作 / 窗口 | 0/10 / 0/10 | **4/10 / 4/10** |
| whole+partial 首轮 whole 精确 | 120/120 | **120/120** |
| whole→partial / whole→单位事件 | 0 / 0 | **0 / 0** |

新路径出现在 5 个纯局部案例中，5 个操作与窗口都正确；其中 4 个是净新增正确，ZSL192 middle
原本已经正确。关键逐案结果为：

| 案例 | 真 firstFixedYear | 输出 |
| --- | ---: | --- |
| ZSL091 older | 1825 | `partialMove -4`, 1822--1834, Top1 1828 |
| ZSL092 older | 1826 | `partialMove -4`, 1822--1834, Top1 1828 |
| ZSL171 older | 1845 | `partialMove -4`, 1835--1847, Top1 1841 |
| ZSL191 older | 1847 | `partialMove -4`, 1844--1856, Top1 1850 |
| ZSL192 middle | 1907 | `partialMove -4`, 1901--1913, Top1 1907 |

保存重开严格差分为 238/239；唯一的 ZSL172 older 在保存前后都仍是同一个 falseRing、同一
`1834--1846` 窗口和 Top1 1835，差异只在内部审计对象，不是用户可见类型变化。组合层重复引用
该控制，所以显示为 116/120；本轮恢复的 5 个 partial 全部保存重开稳定。

安全门禁：co612 多缺轮、mtr841 和物理连续缺块回归 13/13；物理位移
`-2/-3/-4/-6/-10/-30/-50/-100` 全部保持 partial。B6 完整 415 状态中，新分支触发 0 次，单缺轮控制 240/240，
gap=9 和 gap=21 的操作均为 29/30，clean review 2/55。B7 完整结果位于
`D:\软件测试\co612-operation-composition-results\b7-false-pair-completed-partial-safety-2026-08-09`，
...
### Round 7B：三小时时间盒下的最小 A/B/C 验证

停止 B8--D4 的位置、间距和排列穷举，已有日志与代码均保留。本轮冻结的日常验证只使用
`mon151`、`mon152`，每条样芯只取中部 0.50 与较新侧 0.75；主验收间距固定为 21 年。
间距不超过 4 年标记 `operation-unidentifiable`，5--13 年标记 `operation-stress`，至少 14 年才
进入主验收。这个分层只存在于基准清单和报告中，生产诊断没有按事件间距统一隐藏建议。

基准 schema 升为 4：`benchmark-co612-unit-pulse-composition.ts` 支持两个位置、C3 三事件、第三步
结果、三事件全部应用排列的绝对年份回环，以及 `manifest-only`；whole+local 基准支持两个位置、
跳过重复 clean 和 `manifest-only`。冻结清单为 2 条样芯、2 个位置、gap=21，共 64 个 B4--C3
场景；C1 单独运行 16 个，剩余场景一次运行 48 个。没有针对样芯编号或固定年份修改生产算法。

#### 本轮共用机制

1. 复核层读取同一 key 的最后一条反事实证据。诊断链早期可能留下 `correction=-2`，后续联合
   操作已经验证 `correction=-6`；旧代码读取第一条会把正确 partial 在保存重诊断后拒绝，现以
   最新完成态为准。若最后一条仍不一致则继续拒绝。
2. 对累计负 lag `c`，完整比较直接 `partial(c)` 与
   `partial(c+1) + missingRing(-1)` 的两种日历顺序。断点和幅度联合评分，每条参考芯独立比较
   完整纠正后的 raw、差分和预白化结果；最终仍只输出较新的一个操作和一个 7/9 年窗口。
3. 混合搜索入口可来自两类 partial 投票一致、两个独立 hard-gated 候选、`c -> 0` 与
   `c+1 -> -1` 的单位残余候选对，或一个 hard-gated 候选与附近强投票。入口只允许完整假设参加
   竞争，不能直接绕过逐参考芯家族门槛。
4. 搜索采用旧模式优先的两阶段策略：原搜索模式已经通过门槛时立即返回；只有原模式不通过才
   加入额外粗锚点。早期把所有锚点放入同一 Top24 池曾使 gap=9 的 2 个正确案例变成错误操作，
   两阶段策略将这项退化恢复为 0，同时避免所有案例承担第二次搜索成本。
5. 未通过的组合竞争只保留在离线结果中，不向生产事件附加审计 source 或候选窗口。

#### 不可缩减安全门禁

结果目录：

- `a0-a3-a4-b3-partial6-after-seed-2026-08-09`
- `a3-a4-b3-partial20-after-seed-2026-08-09`
- `a1-a4-b1-whole-missing-minimal-2026-08-09`
- `a2-a4-b2-whole-false-minimal-2026-08-09`

所有源 SHA 前后均为
`36e6c6a9d0cbc16d1870a1662da553a7b40d5578ea9ede25ff790c556c34667d`，错误 0、残差回环错误 0、
保存重开稳定率 100%。clean review 为 **2/55**。纯 whole `-5/+5`、中部/较新侧均为 4/4；
whole+missing、whole+false、whole+partial -6、whole+partial -20 各 8/8 首步精确输出 whole，
whole 转 partial 或单位事件均为 0。纯 missing 为 4/4；纯 partial -6 与 -20 均为 4/4，幅度精确；
纯 false 为操作 3/4、窗口 2/4，作为 A2 未解决项保留。

#### A/B/C 冻结结果

| 类别 | 场景 | 完整通过 | 安全拒答 | 其余失败 | 结论 |
| --- | ---: | ---: | ---: | ---: | --- |
| A0 clean | 55 | 53 clean | 0 | review 2 | 达到 `<=2/55` |
| A1 单缺轮 | 4 | 4 | 0 | 0 | 通过 |
| A2 单伪轮 | 4 | 2 窗口通过 | 0 | 1 类型错误、1 窗口错位 | 未解决 |
| A3 单 partial -6/-20 | 8 | 8 | 0 | 0 | 通过 |
| A4 单 whole -5/+5 | 4/轮 | 4/轮 | 0 | 0 | 通过 |
| A5/B5 远距离双缺轮 | 4 | 4 | 0 | 0 | 通过 |
| B1 whole + 缺轮 | 8 | 8 | 0 | 0 | 通过 |
| B2 whole + 伪轮 | 8 | 4 串行 | 0 | 4 继承 A2 | whole 首步 8/8 |
| B3 whole + partial -6/-20 | 16 | 16 | 0 | 0 | 通过 |
| B4 缺轮 + 伪轮，两顺序 | 8 | 2 | 6 | 0 | 保持安全，召回不足 |
| B6 双伪轮 | 4 | 3 | 0 | 第二步类型错 1 | 未解决 |
| C1 partial + 缺轮 -6 | 8 | 4 | 1 | 累计 -7 误作 partial 3 | 未解决 |
| C1 partial + 缺轮 -20 | 8 | 1 | 4 | 累计 -21 误作 partial 3 | 未解决 |
| C2 partial + 伪轮 | 16 | 1 | 0 | 操作错误 15 | 未解决 |
| C3 partial + 缺轮 + 伪轮 | 16 | 0 | 0 | 首/二步类型或窗口失败 16 | 未解决 |

B4--B6、C2、C3 的完整逐案结果位于
`D:\软件测试\co612-operation-composition-results\b4-b6-c2-c3-minimal-final-2026-08-09`；C1 最终结果分别位于
`c1-minimal-shift6-two-stage-final-2026-08-09` 与
`c1-minimal-shift20-two-stage-final-2026-08-09`。所有这些运行保存重开均为 100%，没有 whole 误判。
汇总 99 个实际显示的非 whole 事件，窗口宽度仅有 5/7/9/13 年，分别为 8/26/22/43 个，
超出允许宽度的事件为 0。

gap=9 的 optional-success 回归位于
`optional-c1-gap9-two-stage-retention-2026-08-09`：旧版本已正确的 3 例保持 3/3，正确转拒答 0，
正确转错误操作 0；原本拒答的 3 例仍安全拒答。由此可冻结当前版本，但不能宣称 C1--C3 已普遍
解决。下一轮应首先处理 A2/B6 的伪轮窗口与类型，再以同一最小清单检查 C2；不得恢复旧矩阵或
根据本轮失败临时选择容易年份。

### Round 7C：共享零值不得翻转明确的伪轮方向

A2 的 `mon152` 中部与 B6 的 `mon151` 第二步具有同一失败机制：内部已经形成候选支持的
`falseRing (lag +1 -> 0)`，但后置的 sequential-missing 路径在参考芯中找到 35 个共享零值，
把正向单位事件覆盖成 `missingRing`。该路径的起始 lag 分别达到 -55 与 -48，并不是目标序列
自身的缺轮方向证据。

现将“共享参考零值”降为定位证据：当当前事件集只有明确的 falseRing 时，它不能单独翻转操作
方向。只有目标自身已确认的零值阶梯、missingRing 候选或已检测到的 missingRing 才能覆盖该
正向单位事件。没有相反单位事件时，共享零值仍可用于原有多缺轮阶梯定位，不改变 CO612
`mon052` 与 `mtr841` 的逐轮恢复路径。

冻结的小范围结果为：

- `a2-direction-gate-smoke-2026-08-09`：2 条样芯、2 个位置、4 个单伪轮；操作由 3/4 提升为
  4/4，窗口由 2/4 提升为 3/4，whole 对照和 whole+false 均 100%，保存重开 100%。唯一剩余
  失败是 `mon152` 较新侧，真值 1891，输出 falseRing 1881--1889，属于窗口向旧侧偏 2 年，
  不再是类型错误。
- `b6-direction-gate-smoke-2026-08-09`：4 个 gap=21 双伪轮场景全部两步完成；首步与第二步
  操作、窗口均 4/4，错误方向 0，最终残余建议 0，保存重开 100%。此前唯一的第二步
  `falseRing -> missingRing` 已消失。
- `b4-direction-gate-retention-2026-08-09`：原有 2/8 完整成功全部保持，6/8 仍安全拒答；
  正确转拒答 0、正确转错误操作 0，没有用错误方向换取召回。

以上三项源文件 SHA-256 前后均为
`36e6c6a9d0cbc16d1870a1662da553a7b40d5578ea9ede25ff790c556c34667d`，错误 0，保存重开稳定率
100%。事件集成单元测试 55/55；CO612 `mon052` 九轮恢复与 `mtr841` 全部自然零逐轮恢复均通过。
ZSL 文件级用例本轮未进入断言：测试夹具的 RAW OUT 文本为 null，在 `splitReportByParts` 初始化
时失败，单列为夹具问题，不计作本机制回归失败。

一次通过 `npm run ... -- --flags` 的调用被 npm 抹去参数名，误执行了 wrapper 默认矩阵。该结果
只保留为非冻结日志，不纳入本轮 A/B/C 验收；它未修改源 RWL。后续一律先用直接 `node` 入口的
`--manifest-only true` 核对案例数，再完整串行执行。该非冻结日志中 clean review 仍为 2/55，
20 个单伪轮操作为 20/20，whole 40/40 且 whole 转 partial/unit 均为 0，可作为额外安全观察，
但不能替代上述冻结清单。

### Round 7D：连续同向边缘证据重置伪轮主窗重心

Round 7C 后 A2 只剩 `mon152` 较新侧的窗口错误：真值 1891，操作已正确为 falseRing，但主窗
1881--1889 向旧侧偏移。该例不是单一峰值右移规则，而是完整定位链连续两次给出同向边缘证据：

1. joint counterfactual 将 1880--1886 推进为 1881--1887，Top1 1887 仍位于较新边缘；
2. edge rank guard 扩成 1881--1889，同时形成内部续接范围 1885--1891；
3. 当前 9 年窗的 Top1 比中心偏新 2 年，两个 7 年前态也恰好同向平移 1 年。

只有上述三项同时成立，且确实存在同向 continuation location 时，才把唯一 9 年主窗以 Top1
为重心移动 2 年。该步骤在 alternatives 清理前执行，最终仍删除 location alternatives，不向 UI
显示备选窗口。一次只出现 edge guard、缺少前序 joint advance 或没有 continuation 的事件保持
原窗不动。离线检查 20 个单伪轮日志时仅 1 例满足门槛，覆盖从 17/20 增至 18/20，已有覆盖
丢失为 0。

冻结结果：

- `a2-continued-edge-recenter-wired-2026-08-09`：A2 操作 4/4、窗口 4/4；目标例输出
  falseRing 1883--1891，Top1 1887。whole 与 whole+false 均 4/4，保存重开 100%。
- `b6-edge-recenter-retention-2026-08-09`：B6 四场景仍全部两步完成，首步/第二步窗口均 4/4，
  错误方向 0、残余建议 0。
- `b4-edge-recenter-retention-2026-08-09`：B4 仍为 2/8 完整成功、6/8 安全拒答；正确转拒答 0，
  正确转错误操作 0。

三项均错误 0、保存重开稳定率 100%，源 RWL SHA-256 前后仍为
`36e6c6a9d0cbc16d1870a1662da553a7b40d5578ea9ede25ff790c556c34667d`。该机制只改变已输出事件
的窗口，不参与候选生成、复核门槛或拒答恢复，所以 clean review 的事件数量保持既有 2/55，
不会因该步骤新增误报。A2 与 B6 至此从最小 A/B/C 未解决项转为通过；下一步再进入 C2，仍只跑
2 条样芯、2 个位置、gap=21 的 16 个最小场景。

### Round 7E：在最终定位后复核 partial + false 完成态

C2 基线中，真实 `partial(p) + falseRing(+1)` 经常被全区间融合压成一个累计
`partial(c)`，其中 `c = p + 1`。典型错误是把真实 `-6/-20` 输出成 `-5/-19`。本轮没有按
样芯或年份改规则，而是把已有的完成态纠正竞争推广到正向单位事件：

1. 对累计位移 `c` 同时比较直接 `partial(c)` 与
   `partial(c-1) + falseRing(+1)` 的两种日历顺序；局部移动较新时先移动旧侧，再在变换后的
   年份删除伪轮，伪轮较新时先删年，再把旧侧 `firstFixedYear` 加 1。
2. 比较器在全区间定位和动态操作融合之后运行，因而看到的是最终 `-5/-19`，而不是定位前的
   临时事件。入口要求最终 partial 与同幅 hard-gated partial 候选一致，或同轮存在明确的
   `falseRing (+1 -> 0)` hard-gated 候选；候选只负责建立搜索入口，不能绕过完成态门槛。
3. 每条参考芯分别比较完整纠正后的 raw、差分和预白化得分，并要求 mixed family 与两种事件
   顺序具有独立优势。通过后只输出较新的一个前沿操作、一个 7/9 年窗口和一个 Top1；不显示
   第二窗口或备选位移。
4. 若输出的前沿是 partial，证据必须追加最终 `counterfactual_correction_years=p`。源累计事件
   的旧 `c` 注记仍保留作审计，但 review 层读取最后一条纠正幅度；只有最终幅度、lag 与操作
   一致时才允许显示。

正式结果位于
`D:\软件测试\co612-operation-composition-results\c2-post-locator-review-fixed-v4-2026-08-09`。
与同一清单的 `c2-after-a2-b6-fixes-2026-08-09` 相比：

| 指标 | 修复前 | 修复后 |
| --- | ---: | ---: |
| 首轮响应 | 16/16 | 16/16 |
| 首轮操作正确且窗口覆盖 | 1/16 | **5/16** |
| 第二事件窗口覆盖 | 1/1 | **5/5** |
| 完整串行恢复 | 1/16 | **5/16** |
| 保存重开稳定 | 16/16 | 16/16 |
| 正确转拒答 | 0 | 0 |
...
新增成功为 `mon151 partialThenFalse -6 middle`、`mon152 falseThenPartial -20 middle`、
`mon152 falseThenPartial -6 middle` 与 `mon152 partialThenFalse -6 middle`；原有
`mon151 falseThenPartial -6 newer` 保持完整成功。按幅度，`-6` 为 4/8，`-20` 为 1/8；
按位置，中部为 4/8，较新侧为 1/8。5 个首窗全部覆盖真值，应用第一步后第二窗也全部覆盖，
最终残余建议为 0。源文件 SHA-256 前后均为
`36e6c6a9d0cbc16d1870a1662da553a7b40d5578ea9ede25ff790c556c34667d`，错误 0。

本轮仍不能把 C2 标为通过。剩余 11 例中有 10 例仍把累计 `-5/-19` 当作 partial，1 例把
组合误作 missingRing；尤其较新侧缺少同幅硬候选，`-20` 的 mixed family 在多数参考芯上也
没有胜过直接 partial。临时“见到组合就放行”的旁路最多只得到 4/8 且产生错误顺序/窗口，已
撤销且不纳入冻结结果；后续不能靠降低 family 门槛处理这些案例。

安全门禁结果：

- `safety-clean-a3-b3-partial6-c2-v4-2026-08-09`：clean review 仍为 **2/55**；A3 `-6`
  操作、幅度和窗口 4/4，whole 与 B3 4/4，whole 转 partial/unit 为 0。
- `safety-a3-b3-partial20-c2-v4-2026-08-09`：A3 `-20` 操作、幅度和窗口 4/4，whole 与
  B3 4/4。
- `safety-a2-b2-c2-v4-2026-08-09`：A2 操作和窗口 4/4，whole 与 whole+false 4/4。
- `safety-b6-c2-v4-2026-08-09`：B6 仍为 4/4 两步完整，正确转拒答/错误操作均为 0。
- `safety-b4-c2-v4-2026-08-09`：B4 仍为 2/8 完整、6/8 安全拒答，没有新增错误操作。
- `safety-c1-c2-v4-2026-08-09`：C1 保持 `-6` 4/8、`-20` 1/8；原有 5 个完整成功全部
  保持，正确转拒答/错误操作均为 0。
