...
- [src/features/crossdating/diagnosis/eventApply.ts](src/features/crossdating/diagnosis/eventApply.ts)：把用户复核事件转换为既有编辑语义；默认计划器继续拒绝后台自动执行 `reviewOnly`，建议卡中的显式“应用”通过独立的手动确认入口只解除该门，其余 stale、窗口、方向和范围检查保持不变。局部移动的公开年份是 `firstFixedYear`，应用范围严格结束于 `firstFixedYear - 1`，且自动位移必须小于 -1。
...
- 宽度网格的序列级虚拟化只在 overscan 后的可见序列索引范围变化时更新 React viewport 状态；在同一长序列内部滚动不得按每个 `scrollTop` 重渲染全部格子。格子左右插入反馈只在 `mouseenter` 时缓存一次边界，后续 `mousemove` 禁止重复 `getBoundingClientRect()`；默认 outline 必须透明，避免悬停时从 `currentColor` 黑色闪变为灰色。
- 单年插入和单年删除依赖 React 18 自动批处理，把编辑器数据变化与 animation plan 放进同一次事件提交，不使用 `flushSync` 强制渲染整个 Home/Width 树。范围删除、删除恢复和其它依赖前后 DOM 快照的复杂路径仍可保留既有同步提交，未经单独验证不得一并移除。
