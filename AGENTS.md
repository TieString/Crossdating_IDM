...
- 打开文件前先完成待写入的历史草稿；用户选择“磁盘文件（放弃缓存）”后必须立即持久化磁盘版本和空历史，不能等到下一次编辑或保存才覆盖旧草稿。
- 同输入在途验证只运行一次。
...
- 设置页“缓存管理”只安排下次主窗口启动前清理，允许取消。默认清理COFECHA临时文件、报告和扫描影像副本；全部文件工作区记录（含未保存草稿、参考配置、扫描标注）必须显式勾选并确认。清理必须在React挂载及旧缓存迁移前执行，只遍历应用专用目录、不跟随符号链接、不删除源文件或设置。

## RWL与数据保护

- Tucson短格式和长格式必须保持格式透明：打开什么格式，保存后仍是什么格式。
- Tucson的`999`与`-9999`标识每个终止段的精度（0.01/0.001 mm）；允许同文件混合。解析混合文件时统一工作单位并保存逐序列导出精度，不得把旧终止符变成测量值。显式0保留为缺轮观测，细精度序列中的真实999不能删除。
...
- 扫描影像缓存预算为可配置2–128 GiB（默认16 GiB），按最近使用时间淘汰；Rust读取保护跨窗口共享，前端readFile结束或窗口销毁后释放。TIFF转换/副本创建在后台串行执行，完整文件才可进入缓存。前端Blob必须先登记所有等待者引用再淘汰，清空工作区时活动URL延迟到最后一个使用者释放才回收。
