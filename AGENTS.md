...
- [src/features/crossdating/diagnosis/pathFixedSideWholeBaseline.ts](src/features/crossdating/diagnosis/pathFixedSideWholeBaseline.ts)：当局部 lag 路径的较新固定侧仍为非零 lag 时，将该终态恢复为 whole 草案；依次执行连通局部链与 whole 的联合内存反事实，再通过普通或组合 hard gate。通过的候选必须由 supplemental sink 返回引擎，保证 UI 显示的 whole 事件具有可实际应用的 candidate ID。
...
- [src/features/crossdating/diagnosis/reviewWindowDisplay.ts](src/features/crossdating/diagnosis/reviewWindowDisplay.ts)：复核显示层优先保留严格层单位事件，其次保留严格局部移动，最后允许单独的严格整体移动；不得因操作类型不同而隐藏已确认的严格局部事件。降低门槛的拒答恢复仍只处理单位缺轮/伪轮，不生成低置信局部移动。
...
- [src/pages/home/BreadthDiagnosisNavigator.tsx](src/pages/home/BreadthDiagnosisNavigator.tsx) 与 [src/pages/home/breadthDiagnosis.ts](src/pages/home/breadthDiagnosis.ts)：主窗口右上角是全文件广度复核导航器。当前选中序列仍由高优先级深度诊断负责；后台 worker 逐条扫描其他序列，只收集通过 review 显示门槛的唯一窗口，COFECHA 标记序列优先计算，展示顺序按窗口首次进入队列的 FIFO 时间。编辑、参考或 COFECHA 状态变化只会令结果失效；仅在成功保存且该次 COFECHA 结束后启动一轮扫描，当前诊断、保存、载入和 COFECHA 运行期间后台扫描暂停。
