...
- [src/features/crossdating/diagnosis/unitEventShortWindowSelector.ts](src/features/crossdating/diagnosis/unitEventShortWindowSelector.ts)：学习定位器先锁定唯一 9/13 年模式；只有独立物理定位窗完整嵌套在已通过安全门的 9 年窗内时，才按冻结的事件特异门缩到 7/5 年。缺轮 5 年窗还要检查当前主年份、操作锚点和单侧阶跃锚点的中位位置，锚点落在窗边时回到 9 年；13 年困难案例不得直接缩窄。
