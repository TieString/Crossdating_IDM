...
- [src/components/Chart/MultiLineChart.tsx](src/components/Chart/MultiLineChart.tsx)：以加粗红色实线绘制手动 reference series，以绿色虚线绘制 COFECHA-pass 动态参考；参考数据集先于普通/目标序列绘制，保持在底层，并显式保留空缺年份断点、在 tooltip 中显示 sample depth。样本量曲线对有效样本数为 0 的年份写入 `null` 并断线，尤其不得在 RWL 终止符年份从最后真实样本突然降到 0；样本量控件位置维持既有布局。普通序列先使用不含参考红色的高差异种子色，超出后由 [src/components/Chart/seriesColors.ts](src/components/Chart/seriesColors.ts) 以低差异候选流和 OKLab 最大距离继续生成，不循环复用已有颜色；当前可见序列的 flagged/problem segments 作为淡色背景带显示。
- 主窗口尚未打开文件时，右侧 COFECHA 文本区和图表内容区保持空白，但固定工具栏继续显示；骨架屏只用于真实文件加载阶段，图表骨架不绘制模拟折线。左下问题/定年建议模块只在用户选中一条具体序列后显示，未选序列或处于“全部”时宽度数据区占满左侧。
...
- 应用手选复核年份前先清除图表局部预览，再允许编辑器同步发布新 working data；若旧 `SHIFT_RANGE` 预览在过渡帧遇到目标年份冲突，预览层返回空而不抛出 `RwlMoveConflictError`，最终编辑仍走事件计划的严格冲突检查。
- “待复核序列”广度扫描只把最新 COFECHA PART 6 带 `A` 标记的 `candidateFlaggedIds` 放入队列；总数、进度、重试和建议列表均使用该集合，非 A 序列不再仅因其他问题记录或历史优先级进入扫描。建议列表固定为两行高度，全部优先级结果保留在内部滚动区，超过两条时用滚轮查看，不再改变模块高度。
- 切换当前序列触发前台诊断时，仍有待扫描 A 目标才把后台状态标为暂停，按钮显示“已暂停”而不是“扫描中”；没有可运行目标时保持完成状态，避免切换序列造成状态闪烁。
...
- [src/components/WidthContainer/treeRingArtwork.ts](src/components/WidthContainer/treeRingArtwork.ts)：按 Tucson 终止符同步换算 RWL 宽度单位（`999` 为 `0.01 mm`，`-9999` 为 `0.001 mm`），复现早材白底、六层晚材点纹和 0.18 mm 年轮边界；文件载入后按空闲时间分片预热各序列，并以精度、宽度签名和有界 Blob URL 缓存一份完整物理截面，径向预览与完整悬浮窗复用该 SVG。每个 `series-header` 中间的窗口占满标题统计项以外的剩余宽度；预览以 10 mm 为基准，再按 SVG 实际像素比例动态扩展或收窄垂直物理视窗，使树心到树皮始终完整、四边填满且不发生横纵独立拉伸。动态 `viewBox` 提供 1–32 倍滚轮缩放与横向拖动，不需要逐帧重绘 Canvas；年轮上的滚轮由非被动捕获监听消费，不允许继续滚动工作区。悬停年份提示显示在按钮上方，单击只选择对应宽度格并标出该轮，不改变影像当前视角。双击继承当前 header 类型：绘制横条打开 1 cm，扫描横条打开扫描视图；唯一悬浮窗提供“1 cm / 扫描 / 完整”三视图且“完整”固定在最右侧。绘制版两种视图都支持悬停年份与单击定位宽度格；只有直接点击宽度格或外部定位才允许反向聚焦影像，影像自身单击不得居中。滚轮缩放、放大后平移和双击重置只作用于当前视图。标题栏负责移动窗口，右下角手柄可调窗口大小且始终限制在可视区域。影像右键菜单直接复用宽度右键菜单的容器、行、图标、选中态和弹出动画样式，并可在悬浮窗上方显示。中间缺失年份以不虚构物理宽度的分隔标记显示，显式 0 宽缺轮单独标记；编辑后的当前序列使用新精度和宽度签名自动更新。宽度模块不再显示贝叶斯定年按钮，旧算法实现文件暂予保留。
...
- [src/components/WidthContainer/useTreeRingScanImage.ts](src/components/WidthContainer/useTreeRingScanImage.ts) 与 Tauri `prepare_tree_ring_scan_image` 命令：外部扫描图按需复制到 app cache；大型 TIFF 使用分块读取生成总览，用户自由选框后按原始像素提取样本截面，不静默降采样。完整扫描窗口和 `series-header` 横条都使用高 DPI Canvas，缩放、平移或旋转时直接从原始裁切图重绘当前视口，禁止放大低分辨率 SVG 中间层。header 横条高 32 px、无额外内边距，低 DPI 屏幕至少以 2× 后备分辨率渲染；高倍缩放允许亚像素源视窗，并只使用单一等比缩放系数，禁止以 1 px 最小高度造成横纵变形。缩放上限至少为 64 倍，并按源图可用像素自适应扩展（安全硬上限 1024 倍），超过原生像素密度后关闭插值以显示真实像素。锚点与年份文字在不参与影像缩放的独立屏幕坐标层渲染，禁止使用父层放大/子层反向缩小。前端 Blob URL 缓存有数量和字节上限，Rust 缓存按 2 GiB 上限清理旧文件。支持 SVG、PNG、JPEG、WebP、BMP、GIF 和 TIFF。扫描图选择、截面选框、90° 旋转、锚点、校准基线与显示模式按 RWL 文件路径持久化，重启后恢复。
- [scripts/export-tree-ring-scan-fixtures.ts](scripts/export-tree-ring-scan-fixtures.ts)：按输入 RWL 的终止符精度，把全部序列导出为与序列 ID 同名的完整绘制版 SVG，可直接作为扫描影像工作流的文件夹测试夹具。
...
- 删除标记 hover ghost 支持右键菜单，菜单仅含“删除该标记”，双击仍执行恢复。一次范围删除产生的标记共享 `operationGroupId`，右键或双击任一当前顶层标记会在一次可撤销操作中删除或恢复该批次全部标记；逐次单年删除及旧快照标记没有分组，仍只处理当前一层。右键只移除标记、不恢复数据。
