...
- **定义**：特殊值，表示数据中断，并同时约定 Tucson 整数宽度的物理精度
- **精度约定**：
  - `999`：每个宽度整数单位为 `0.01 mm`
  - `-9999`：每个宽度整数单位为 `0.001 mm`
- **作用**：标记时间序列的结束或中间缺口
- **处理**：
  - 读取：先按位置、后续行和十年边界确定终止身份；内部真实999保留。完整规则见[README](README.md#rwl结束符真实999与分段规则)。
  - 存储：工作数据统一使用0.001 mm和负数终止标记；来源段的终止位置及单位另存为`tucsonSegments`。
  - 保存：按段恢复可无损表达的来源精度，真实999不作数值过滤；结束符由导出器重新生成。
  - 绘制：按工作数据的单位换算为毫米，而不是直接使用原文件整数的单位。
