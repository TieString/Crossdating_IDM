...
export default function ({ siteData: site, masterSeries, selected }: { siteData: RwlSiteData, masterSeries?: Map<number, number>, selected?: string }): ReactNode {
                ...
                Array.from(site).map(([key, value]) => {
                    ...
                    return Array.from(value).map(([year, width]) => (
                        <Fragment key={`${key}-${year}`}>
                            ...
                            <WidthGrid gridValue={width} year={year} masterSeriesValue={masterSeries?.get(year)} />
