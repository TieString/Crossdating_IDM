...
describe("false-ring mode selector", () => {
    it("selects one deterministic 13-year mode from concentrated evidence", () => {
        ...
        const first = selectFalseRingMode(input);
        const second = selectFalseRingMode(input);
        ...
        expect(first!.scoredWindows.length).toBeGreaterThan(1);
    });

    it("does not select a mode for a sequence shorter than 13 years", () => {
        expect(selectFalseRingMode({
