...
需要为未定年序列搜索绝对年份时，在 COFECHA 工具栏点击“加载未定年”。当前工作区作为 dated master，报告新增 PART 8；结果可按最高相关 `R` 或年份调整 `D` 排序。
...
- **双 COFECHA 引擎**：内置 `cofecha-js`，也可加载用户从 LTRR 获取的官方 EXE；统一支持 PART 导航、未定年序列 PART 8、问题段定位、完整 OUT 导出和 COFECHA-pass 动态参考。
