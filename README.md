...
同名多段存入同一个按年份索引的Map，来源段边界、终止位置和单位另存为`tucsonSegments`。空缺年份不补0、不插值、不移动后续年份；显式0仍表示缺轮。内部工作数据统一使用0.001 mm整数和负数终止标记，计算链保持原有单位。宽度网格按对应段的导出精度显示：例如999来源段的内部7060显示为706，输入707后写入内部7070。结束符同样按来源显示，真实999保持可编辑；序列标题及格子提示标明单位。

显示转换覆盖网格、删除标记、数字动画、查找替换、年/值文本编辑、控制台复制及操作记录中的数值。有效修改CSV的单值修改使用明确的显示单位；无法可靠归因的区间仍按标明的工作单位列出。内部日志、持久化基线和COFECHA证据不随显示转换改写。折线图曲线坐标保持内部值，宽度轴刻度换算成毫米，提示框显示各段原数值和单位；独立图表与日志窗口同步相同显示信息。
