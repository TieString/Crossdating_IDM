#!/usr/bin/env python3
"""Freeze every untouched reserve file as one development extension set."""
...
def main() -> None:
    ...
    qualified = json.loads(
        Path(args.qualified_pool).read_text(encoding="utf8")
    )["qualified"]
    reserve_ids = [
    ...
    if not reserve_ids:
        raise RuntimeError("frozen split has no reserve files")
    if any(file_id not in by_id for file_id in reserve_ids):
        raise RuntimeError("reserve file missing from qualified pool")

    config = copy.deepcopy(base)
    config["frozenDate"] = "2026-08-27"
    config["seed"] = "standalone-unified-v2-reserve-development-2026-08-27"
    config["fileIds"] = reserve_ids
    config["design"]["datasetRole"] = "developmentExtension"
    config["design"]["splitId"] = (
        "standalone-unified-v2-pre-frozen-reserve-development-2026-08-27"
    )
    config["statistics"]["seed"] = (
        "standalone-unified-v2-reserve-development-bootstrap-2026-08-27"
    )
    ...
    manifest = {
        ...
        "sourceFrozenSplit": str(Path(args.frozen_split).resolve()),
        "sourceRole": "reserve",
        "datasetRole": "developmentExtension",
        ...
        "counts": {
            ...
            "excludedFiles": 0,
            "totalSeries": sum(
                by_id[file_id]["totalSeries"] for file_id in reserve_ids
            ),
            ...
            "eligibleTargets": sum(
                len(by_id[file_id]["eligibleTargets"])
                for file_id in reserve_ids
            ),
        },
    }
