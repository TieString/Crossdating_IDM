...
`RWL -> 解析目标/参考序列 -> 当前轮 target -> leave-one-out master -> selected top500 -> 251 项部署特征 -> LambdaRank -> 可靠性拒答 -> Top5 中心及窄范围`

Tauri 只负责文件选择、序列选择、会话中的人工确认记录、进程生命周期和结果展示。模型不会自动改写 RWL。

## 2. 资源

模型目录：`artifacts/exports/current_event_tauri_bundle_v1_20260716/bundle/`
...
python scripts/current_event_ranker_sidecar.py `
  --bundle artifacts/exports/current_event_tauri_bundle_v1_20260716/bundle
```
...
`status=advice` 时显示 `suggestions`。每条包含中心年份、默认 ±1 年范围、排序分数和可解释证据。

`status=evidence_insufficient` 时不得把低可靠候选包装成可采纳建议。常见原因包括参考序列不足、重叠不足、无候选或可靠性低于阈值。

`rankingScore` 只是同一轮候选之间的相对排序分数，不是概率。`reliability.score` 是本轮是否适合给出 Top5 建议的选择器输出，也不是某个年份为真的概率。
...
发布前必须执行：模型包哈希校验、`health`、`describe`、模式顺序校验、预测参考一致性、同一请求重复预测一致性、两轮 JSONL 会话、结构化错误测试。

## 7. 当前质量边界

部署可用模型在固定确认3+4的 831 轮中，可靠性接受 598 轮，接受集精确 Top1/Top5 为 0.5920/0.8127，Top5 中心 ±1 年覆盖为 0.8846；接受文件覆盖 87.45%，source_file 宏平均 ±1 为 0.8523。
